...
void Campionat::adauga_echipa(const Echipa& e)
{
    echipe.push_back(e);
}
...
void Campionat::genereaza_program()
{
    ...
    for(int etapa_index=1; etapa_index<=etape_tur; etapa_index++)
    {
        ...
        std::vector<Echipa*>noua_rotatie;
...
void Campionat::afiseaza_cel_mai_bun_jucator_echipe()const
{
    ...
    {
        std::cout<<e.getnume()<<":";
        std::shared_ptr<Jucator>j=Echipa::celmaibun(e.getlot());
        if(j!=nullptr)
        {
            std::cout<<*j<<'\n';
            std::cout<<'\n';
...
void Campionat::afiseaza_clasament()const
{
    ...
    std::sort(clasament.begin(),clasament.end(),[](const Echipa& e1,const Echipa& e2)
    {
        bool rezultat;
...
Campionat Campionat::citeste_campionat(std::istream& f)
{
    ...
    {
        c.adauga_echipa(Echipa::citeste_echipa(f));
    }
    return c;
}
